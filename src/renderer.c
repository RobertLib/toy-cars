...
static const char *world_fs = GLSL CLOUD_GLSL
    ...
    "vec3 uEye,uFog;uniform float uFogFar,uUnlit,uOvercast;\n"
    ...
    "base=vColor; if(vGround>.5){float patch=terrainNoise(vPos.xz*.13);"
    ...
    "spec=pow(max(0.,dot(n,halfV)),48.)*.12*sh*sun;col+=spec;col=mix(col,vColor,uUnlit);float "
