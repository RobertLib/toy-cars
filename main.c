...
static bool SDLCALL lifecycle_watch(void *data, SDL_Event *e) {
    ...
    if (e->type == SDL_EVENT_WILL_ENTER_BACKGROUND)
        SDL_SetAtomicInt(&l->background, 1);
    if (e->type == SDL_EVENT_DID_ENTER_FOREGROUND)
        SDL_SetAtomicInt(&l->background, 2);
    return true;
}
...
int main(int argc, char **argv) {
    ...
    int track = 0, frames = 0, width = 1440, height = 900;
    ...
    for (int i = 1; i < argc; i++) {
        if (!strcmp(argv[i], "--test"))
            ...
            printf("ToyCars\n  --test                     Run deterministic simulation tests (no "
                   ...
                   "menu|race|garage|help|settings|results\n  --autoplay                 Drive the "
    ...
    while (running) {
        ...
        if (life_event) {
            background = life_event == 1;
            if (background && (g->screen == SCREEN_RACE || g->screen == SCREEN_COUNTDOWN))
                g->screen = SCREEN_PAUSE;
            ui_clear_input(u);
