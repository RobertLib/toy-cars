...
static void test_props_rendering(Renderer *r, const Game *g) {
    ...
    for (int track=0;track<TRACK_COUNT;track++) {
        scene->selected=track;
        game_start(scene);
        EXPECT(scene->prop_count==24, "each track has six cones and eighteen tires");
        ...
        for (int view=0;view<2;view++) {
            Car *c=&scene->cars[0];
            c->pos=c->prev=view ? scene->props[9].pos : scene->roadwork;
...
static void test_water_rendering(Renderer *r, const Game *g) {
    ...
    for (int track = 0; track < TRACK_COUNT; track++) {
        ...
        EXPECT(ford >= 0, "water preview locates an actual ford");
        if (ford < 0) continue;
