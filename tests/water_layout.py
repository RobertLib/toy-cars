...
for theme, name in enumerate(('country', 'beach', 'winter')):
    ...
    assert wet, (name,'missing ford')
    groups=1+sum(b!=a+1 for a,b in zip(wet,wet[1:]))
    assert groups==1, (name,'unexpected road flooding',wet)
    ford=path[wet[len(wet)//2]]
    assert math.hypot(ford[0]-route[0][0],ford[2]-route[0][2])>20, (name,'spring beside road')
    assert math.hypot(ford[0]-route[-1][0],ford[2]-route[-1][2])>25, (name,'pool beside road')
    assert len(wet)<14, (name,'ford has become a flooded road section')
    ...
    for obj in bpy.context.scene.objects:
        ...
        if (y:=water_at(center.x,-center.y)) is not None:
            assert center.z<y-.015, (name,'raised curb across watercourse')
    if theme==0:
        ...
        assert min(math.hypot(v.x-p[0],-v.y-p[2])-p[5]
                   for v in pond_vertices[::10] for p in path)>2, 'pond must be separate from road'
    else:
    ...
    print(f'{name}: sinuosity {distance/chord:.2f}, width {min(widths):.2f}–{max(widths):.2f}, '
          f'continuous downhill channel, one ford at {wet}, '
